 ...
 * 
 * @author tag
 * @version $Id: BasicMercatorTiledImageLayer.java 13201 2010-03-12 01:59:03Z
 *          tgaskins $
 */
public class BasicMercatorTiledImageLayer extends MercatorTiledImageLayer implements ExportableLayer{
	private final Object fileLock = new Object();
	private AVList params = null;

	/**
	 * 如果你使用这个构造函数可能会无法导出使图层
	 * @param levelSet
	 */
	...
	private static class RequestTask implements Runnable, Comparable<RequestTask> {
		...
		//	System.out.println(textureURL.getPath());
...
//		try {
//			System.out.println(tile.getResourceURL());
			...
			try {
				...
				if (buffer != null) {
					...
					if (contentType.contains("xml") || contentType.contains("html") || contentType.contains("text")) {
						...
						// ..convertToDDS(buffer, contentType);
						if (buffer != null)
							this.layer.saveBuffer(buffer, outFile);
