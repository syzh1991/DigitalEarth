package org.gfg.dartearth.layer.mercator;
 ...
 * 
 * @author tag
 * @version $Id: MercatorTiledImageLayer.java 13339 2010-04-25 23:16:04Z
 *          tgaskins $
 */
public abstract class MercatorTiledImageLayer extends AbstractLayer {
	...
	private boolean atMaxLevel(DrawContext dc) {
		...
		Sector centerSector = nextToLast.computeSectorForPosition(vpc.getLatitude(), vpc.getLongitude(), this.getLevels().getTileOrigin());
	...
	public int computeLevelForResolution(Sector sector, Globe globe, double resolution) {
		...
		Logging.logger().info(Logging.getMessage("layers.TiledImageLayer.LevelSelection", targetLevel.getLevelNumber(), texelSize));
		return targetLevel.getLevelNumber();
	}

	public BufferedImage composeImageForSector(Sector sector, int imageWidth, int imageHeight, int levelNumber, String mimeType,
		...
		if (levelNumber < 0) {
			...
			Logging.logger().warning(
					Logging.getMessage("generic.LevelRequestedGreaterThanMaxLevel", levelNumber, this.levels.getLastLevel()
							.getLevelNumber()));
			levelNumber = this.levels.getLastLevel().getLevelNumber();
		}
			...
			for (MercatorTextureTile tile : row) {
				...
				try {
					...
					double dh = imageHeight
							* (-tile.getSector().getMaxLatitude().subtract(sector.getMaxLatitude()).degrees / sector.getDeltaLat().degrees);
					double dw = imageWidth
							* (tile.getSector().getMinLongitude().subtract(sector.getMinLongitude()).degrees / sector.getDeltaLon().degrees);
	...
	private MercatorTextureTile[][] getTilesInSector(Sector sector, int levelNumber) {
		...
		for (int row = nwRow; row >= seRow; row--) {
			for (int col = nwCol; col <= seCol; col++) {
				...
				MercatorSector mSector = MercatorSector.fromSector(tileSector); // TODO:
																				// check
