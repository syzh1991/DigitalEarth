...
public class CopyOfLayerPanel2 extends JPanel {
	 ...
	 */
	public CopyOfLayerPanel2(WorldWindow wwd, DartEarthAppFrame frame) {
	...
	protected void fill() {
		...
		for (Layer layer : layerList) {
			...
			if (layerName.matches("HJ1A\\w*_\\w*-\\w*")) {
		...
		JComboBox keyList = new JComboBox(listData);
		...
		ItemListener aListener = new ItemListener() {
			...
			//	buttonGroup = new ButtonGroup();
				...
				for (String date : dateList) {
					...
					LayerAction action = new LayerAction(showLayer,layer, wwd,
							layer.isEnabled());
		...
		for(String date:firstDateList){
			...
			Layer layer = showLayer.get(key);
			LayerAction action = new LayerAction(showLayer,layer, wwd,layer.isEnabled());
			JRadioButton dateRadioButton = new JRadioButton(date);
			dateRadioButton.addActionListener(action);
			buttonGroupPanel.add(dateRadioButton);
		//	buttonGroup.add(dateRadioButton);
	...
	protected static class LayerAction extends AbstractAction {
		...
		public LayerAction(Map<String, Layer> showLayer, Layer layer, WorldWindow wwd, boolean selected) {
			super(layer.getName());
