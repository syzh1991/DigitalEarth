 ...
 * @version $ID$
 */
public abstract class AbstractShape extends WWObjectImpl implements Highlightable, OrderedRenderable, Movable, ExtentHolder,
		GeographicExtent, Exportable, Restorable {
	 ...
	 * shape's altitude mode is other than {@link WorldWind#ABSOLUTE}, the
	 * shape's geometry is created relative to the specified terrain rather than
	 * the terrain used during rendering, which may be at lower level of detail
	 * than required for accurate intersection determination.
	 * 
	 * @param line
	 *            the line to intersect.
	 * @param terrain
	 *            the {@link Terrain} to use when computing the shape's
	 *            geometry.
	 * 
	 ...
	 * renderable list for subsequent rendering. This method is called during
	 * {@link #pick(DrawContext, java.awt.Point)} and
	 * {@link #render(DrawContext)} when it's been determined that the shape is
	 ...
	 *         terminated for this shape. Subclasses should return false if it
	 *         is not possible to create the ordered renderable state.
	 * 
	 ...
	 * be rendered. Called by {@link #makeOrderedRenderable(DrawContext)}just
	 * prior to adding the shape to the ordered renderable list.
	 * 
	 * @param dc
	 *            the current draw context.
	 * 
	 ...
	 * {@link #prepareToDrawOutline(DrawContext, ShapeAttributes, ShapeAttributes)}
	 * , which establishes OpenGL state for lighting, blending, pick color and
	 * line attributes. Subclasses should execute the drawing commands specific
	 ...
	 * {@link #prepareToDrawInterior(DrawContext, ShapeAttributes, ShapeAttributes)}
	 * , which establishes OpenGL state for lighting, blending, pick color and
	 * interior attributes. Subclasses should execute the drawing commands
	 * specific to the type of shape.
	 * <p/>
	 * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
	 * be current when this method is called.
	 * 
	 ...
	 * 
	 * @throws IOException
	 *             if an IO error occurs while writing to the output
	 *             destination.
	 ...
	 */
	protected ShapeAttributes activeAttributes = new BasicShapeAttributes(); // re-determined
																				// each
																				// frame
	...
	protected Position referencePosition; // the location/position to use as the
											// shape's reference point
	protected Object delegateOwner; // for app use to identify an owner of this
									// shape other than the current layer
	...
	protected OGLStackHandler BEogsh = new OGLStackHandler(); // used for
	...
	protected static class AbstractShapeData extends ShapeDataCache.ShapeDataCacheEntry {
		 ...
		 *            the current draw context.
		 * @param minExpiryTime
		 *            the minimum number of milliseconds to use this shape
		 *            before regenerating its geometry.
		 * @param maxExpiryTime
		 *            the maximum number of milliseconds to use this shape
		 *            before regenerating its geometry.
		 */
	 ...
	 * 
	 * @param normalAttrs
	 *            the normal attributes. May be null, in which case default
	 *            attributes are used.
	 */
	 ...
	 *            the highlight attributes. May be null, in which case default
	 *            attributes are used.
	 */
	 ...
	 * 
	 * @param visible
	 *            true to draw this shape, otherwise false. The default value is
	 *            true.
	 * 
	...
	/**
	 * Specifies this shape's altitude mode, one of {@link WorldWind#ABSOLUTE},
	 * {@link WorldWind#RELATIVE_TO_GROUND} or {@link WorldWind#CLAMP_TO_GROUND}
	 * .
	 * <p/>
	 ...
	 * 
	 * @param altitudeMode
	 *            the altitude mode. The default value is
	 ...
	 * the viewer to help eliminate artifacts when two or more faces of this or
	 * other filled shapes are coincident. See
	 ...
	 * {@link #setGeometryRegenerationInterval(int)} for the
	 * regeneration-interval's description.
	 * 
	 ...
	 *            the geometry regeneration interval, in milliseconds. The
	 ...
	 *            the reference position. May be null, in which case the first
	 *            position of the outer boundary is the reference position.
	 */
	...
	protected void determineActiveAttributes() {
		if (this.isHighlighted()) {
			if (this.getHighlightAttributes() != null)
				...
				// If no highlight attributes have been specified we need to use
				// the normal attributes but adjust them
	 ...
	 * the most recent call to {@link #determineActiveAttributes()}. The active
	 * attributes are either the normal or highlight attributes, depending on
	 * this shape's highlight flag, and incorporates default attributes for
	 * those not specified in the applicable attribute set.
	 * 
	 ...
	 *            the current draw context
	 * @param activeAttrs
	 *            the attribute bundle to consider when determining whether
	 *            lighting is applied. May be null, in which case the current
	 ...
	 * @param activeAttrs
	 *            the attribute bundle to consider when determining whether
	 *            normals should be computed. May be null, in which case the
	...
	public void render(DrawContext dc) {
		...
		// to add the shape to the ordered renderable
		// list during pick and render. The third call just draws the ordered
		// renderable.
		...
		// Retrieve the cached data for the current globe. If it doesn't yet
		// exist, create it. Most code subsequently
	...
	protected void checkViewDistanceExpiration(DrawContext dc) {
		// Determine whether the distance of this shape from the eye has changed
		// significantly. Invalidate the previous
		// extent and expire the shape geometry if it has. "Significantly" is
		// considered a 10% difference.
	...
	protected void drawBatched(DrawContext dc) {
		...
		if (!dc.isPickingMode()) {
			...
			while (nextItem != null && nextItem.getClass() == this.getClass()) {
				...
				if (shape.pickLayer != this.pickLayer) // batch pick only within
														// a single layer
					break;
	 ...
	 *            a pick support holding the picked object list to add this
	 *            shape to.
	 */
	protected void doDrawOrderedRenderable(DrawContext dc, PickSupport pickCandidates) {
		GL gl = dc.getGL();
		if (this.getCurrentData().getReferencePoint() != null) {
	...
	/**
	 * Create a {@link gov.nasa.worldwind.pick.PickedObject} for this shape. The
	 * PickedObject returned by this method will be added to the pick list to
	 * represent the current shape.
	 * 
	 * @param dc
	 *            the current draw context.
	 * @param pickColor
	 ...
	 *            the current draw context.
	 * @param attrMask
	 *            an attribute mask indicating state the caller will set. This
	 *            base class implementation sets
	 ...
	 *         use this to set additional state, especially state indicated in
	 *         the attribute mask argument.
	 */
	protected OGLStackHandler beginDrawing(DrawContext dc, int attrMask) {
		GL gl = dc.getGL();
		this.BEogsh.clear();

		// Note: While it's tempting to set each of these conditionally on
		// whether the feature is actually enabled
		// for this shape, e.g. if (mustApplyBlending...), it doesn't work with
		// batch rendering because subsequent
		// shapes in the batch may have the feature enabled.
		attrMask |= GL.GL_CURRENT_BIT | GL.GL_LINE_BIT | GL.GL_HINT_BIT // for
																		// outlines
		...
		gl.glEnableClientState(GL.GL_VERTEX_ARRAY); // all drawing uses vertex
													// arrays
	 ...
	 * @param defaultAttrs
	 *            the attributes to use if <code>activeAttrs</code> does not
	 *            contain a necessary value.
	 */
	 ...
	 * 
	 * @param terrain
	 *            the terrain to compute a point relative to the globe's
	 *            surface.
	 * @param position
	...
	protected Vec4 computePoint(Terrain terrain, Position position) {
		...
		// Raise the shape to accommodate vertical exaggeration applied to the
		// terrain.
	 ...
	 *         returned if either <code>globe</code> or <code>positions</code>
	 *         is null.
	 */
	protected Extent computeExtentFromPositions(Globe globe, double verticalExaggeration, Iterable<? extends LatLon> positions) {
		...
		Extent ext = Sector.computeBoundingBox(globe, verticalExaggeration, sector, minAndMaxElevations[0], minAndMaxElevations[1]);
	...
	protected int countTriangleVertices(java.util.List<java.util.List<Integer>> prims, java.util.List<Integer> primTypes) {
		int numVertices = 0;
		...
		for (int i = 0; i < prims.size(); i++) {
			...
			case GL.GL_TRIANGLE_FAN:
				numVertices += (prims.get(i).size() - 2) * 3; // N tris from N +
																// 2 vertices
				break;
	...
	public void move(Position delta) {
		...
		// The reference position is null if this shape has no positions. In
		// this case moving the shape by a
		// relative delta is meaningless because the shape has no geographic
	...
	protected void exportAsKML(Object output) throws IOException, XMLStreamException {
		...
		// KML does not allow separate attributes for cap and side, so just use
		// the cap attributes.
	...
	protected void doGetRestorableState(RestorableSupport rs, RestorableSupport.StateObject context) {
		// Method is invoked by subclasses to have superclass add its state and
		// only its state
	...
	private void doMyGetRestorableState(RestorableSupport rs, RestorableSupport.StateObject context) {
		...
		// this.highlightAttrs.getRestorableState(rs, rs.addStateObject(context,
		// "highlightAttrs"));
	}
	...
	private void doMyRestoreState(RestorableSupport rs, RestorableSupport.StateObject context) {
		...
		/*
		 * so = rs.getStateObject(context, "highlightAttrs"); if (so != null) {
		 * ShapeAttributes attrs = (this.getHighlightAttributes() != null) ?
		 * this.getHighlightAttributes() : new BasicShapeAttributes();
