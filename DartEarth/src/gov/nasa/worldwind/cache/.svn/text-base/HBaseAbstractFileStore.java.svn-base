 ...
 */
public abstract class HBaseAbstractFileStore extends WWObjectImpl implements FileStore
{
    ...
    public java.net.URL findFile(String fileName, boolean checkClassPath)
    {
        ...
        for (StoreLocation location : this.readLocations)
        {
            ...
            java.io.File file = new java.io.File(makeAbsolutePath(dir, fileName));//这句化很关键，可以把hbase 和普通文件不同的存储合并起来，然后同样处理．
           //TODO this is very important function,if I want to use the hbase I must change this interface
            //只针对一般的文件系统，可以在此处设置一个判断，区别开存储在ＨＢＡＳＥ和文件系统的文件，但是都被封装成为一个file来进行处理
            if (file.exists())
            {
                try
                {
    ...
    //TODO modify
    // result MyDataLayer/MyDataLayer.xml... array
    		...
    		boolean exitBranchOnFirstMatch)
    {
        ...
        // this.readLocations is meaningless in this condition
        // 
        /*
        for (StoreLocation location : this.readLocations)
        {
       ...
       */
        ...
        List<String> layerInfoList = hbaseUtil.getAllLayer(HBaseUtil.TABLE_NAME);
        for(String name :layerInfoList){
    	   String s = hbaseUtil.getLayerConfigFileName(HBaseUtil.TABLE_NAME, name);
    	   nameList.add(name+"/"+s);
    	  // nameList.add(name);
        }
        if(nameList.size() == 0){
        ...
        boolean recurse, boolean exitBranchOnFirstMatch, java.util.Collection<String> names)
    {
        ...
        // this directory.
        //此处还是不需要递归进行判断了吧。。。。因此recurse这 个参数没有必要使用了。
        //下面代码可以不需要了
