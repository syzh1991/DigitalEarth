...
public abstract class WorldWindowImpl extends WWObjectImpl implements WorldWindow
{
    ...
    public void addViewMoveListener(ViewMoveListener listener)
    {
        this.eventListeners.add(ViewMoveListener.class, listener);
    }

    public void removeViewMoveListener(ViewMoveListener listener)
    {
        this.eventListeners.remove(ViewMoveListener.class, listener);
    }

    protected void callViewMoveListeners(final ViewMoveEvent event)
    {
        EventQueue.invokeLater(new Runnable()
        {
            public void run()
            {
                for (ViewMoveListener listener : eventListeners.getListeners(ViewMoveListener.class))
                {
                    listener.moved(event);
                }
            }
