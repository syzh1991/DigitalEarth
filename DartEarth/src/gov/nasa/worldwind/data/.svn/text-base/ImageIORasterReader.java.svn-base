...
public class ImageIORasterReader extends AbstractDataRasterReader
{
    ...
    protected boolean doCanRead(Object source, AVList params)
    {
        ...
        if (o == null || !(o instanceof Sector))
        {
        	...
        	if(source instanceof String){
        		...
        		String tmp = path.substring(0, path.indexOf('/'));
            	String cf = tmp.substring(0,tmp.lastIndexOf('_'));
            	String qualifier = tmp.substring(tmp.lastIndexOf('_')+1);
            	String rowKey = path.substring(path.indexOf('/')+1,path.indexOf('.'));
        		HBaseUtil hbu = new HBaseUtil();
        		boolean ret = hbu.isExist(HBaseUtil.TABLE_NAME, rowKey, cf, qualifier);
        		if(ret == false){
        			return ret;
        		}
        	}
    ...
    protected DataRaster[] doRead(Object source, AVList params) throws java.io.IOException
    {
       // javax.imageio.stream.ImageInputStream iis = createInputStream(source);
    	...
    	///////////////////////zxb////////////////////////////////////////////
    	//source = HJ1A_CCD2_2008-12/0/87/87_213.png
    	...
    	//String cf = tmp.substring(0,tmp.lastIndexOf('_'));
    	//String qualifier = tmp.substring(tmp.lastIndexOf('_')+1);
    	String cf = tmp.substring(0,tmp.indexOf('_'))+"_";
    	tmp = tmp.substring(tmp.indexOf('_')+1);
    	cf = cf + tmp.substring(0,tmp.indexOf('_'));
    	String qualifier = tmp.substring(tmp.indexOf('_')+1);
    	String rowKey = path.substring(path.indexOf('/')+1,path.indexOf('.'));
    	rowKey  = rowKey.replace("/", "\\");
    	
    	HBaseUtil hbu = new HBaseUtil();
    	byte[] bs = hbu.getImage(HBaseUtil.TABLE_NAME, rowKey, cf, qualifier);
