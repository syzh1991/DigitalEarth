 ...
 */
package org.gfg.geohash.zxb.queries;
...
import org.gfg.geohash.zxb.util.GeoHashSizeTable;
...
public class GeoHashBoundingBoxQuery implements GeoHashQuery, Serializable {
	...
	public GeoHashBoundingBoxQuery(BoundingBox bbox) {
		int fittingBits = GeoHashSizeTable
				.numberOfBitsForOverlappingGeoHash(bbox);
		WGS84Point center = bbox.getCenterPoint();
		GeoHash centerHash = GeoHash.withBitPrecision(center.getLatitude(),
				center.getLongitude(), fittingBits);
	...
	private void expandSearch(GeoHash centerHash, BoundingBox bbox) {
		...
		for (GeoHash adjacent : centerHash.getAdjacent()) {
			...
			if (adjacentBox.intersects(bbox)
					&& !searchHashes.contains(adjacent)) {
	...
	private boolean hashFits(GeoHash hash, BoundingBox bbox) {
		return hash.contains(bbox.getUpperLeft())
				&& hash.contains(bbox.getLowerRight());
	}
	...
	public boolean contains(WGS84Point point) {
		return contains(GeoHash.withBitPrecision(point.getLatitude(),
				point.getLongitude(), 64));
	}
	...
	public String getWktBox() {
		return "BOX(" + boundingBox.getMinLon() + " " + boundingBox.getMinLat()
