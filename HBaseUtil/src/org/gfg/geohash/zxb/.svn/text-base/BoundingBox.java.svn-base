...
public class BoundingBox implements Serializable {
	...
	public BoundingBox(WGS84Point p1, WGS84Point p2) {
		this(p1.getLatitude(), p2.getLatitude(), p1.getLongitude(), p2
				.getLongitude());
	}
	...
	// does this bounding box contain with the other bounding box
	public boolean contains(WGS84Point point) {
		return (point.getLatitude() >= minLat)
				&& (point.getLongitude() >= minLon)
				&& (point.getLatitude() <= maxLat)
				&& (point.getLongitude() <= maxLon);
	}

	// does this bounding box have the intersection with the other bounding box
	...
	// get center point in the bounding box
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof BoundingBox) {
			...
			return minLat == that.minLat && minLon == that.minLon
					&& maxLat == that.maxLat && maxLon == that.maxLon;
		} else {
			return false;
		}
	}
