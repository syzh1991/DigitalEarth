...
import org.gfg.geohash.zxb.BoundingBox;
import org.gfg.geohash.zxb.GeoHash;
import org.gfg.geohash.zxb.WGS84Point;
import org.gfg.geohash.zxb.util.BoundingBoxGeoHashIterator;
import org.gfg.geohash.zxb.util.RandomGeohashes;
import org.gfg.geohash.zxb.util.TwoGeoHashBoundingBox;
...
public class GeoHashTest {
	...
	public void testDecode() {
		...
		for (GeoHash gh : RandomGeohashes.fullRange()) {
			...
			assertTrue("bbox " + bbox
					+ " should contain the decoded center value "
					+ decodedCenter, bbox.contains(decodedCenter));
	...
	public void testNotWithin() {
		...
		assertFalse(
				hash.toBase32() + " should NOT be within " + bbox.toBase32(),
				hash.within(bbox));
	}
	...
	public void testByCharacterPrecision() {
		assertEncodingWithCharacterPrecision(new WGS84Point(20, 31), 12,
				"sew1c2vs2q5r");
		assertEncodingWithCharacterPrecision(new WGS84Point(-20, 31), 12,
				"ksqn1rje83g2");
		assertEncodingWithCharacterPrecision(new WGS84Point(-20.783236276,
				31.9867127312312), 12, "ksq9zbs0b7vw");
		...
		for (int characters = 12; characters > 1; characters--) {
			assertEncodingWithCharacterPrecision(point, characters,
					fullStringValue.substring(0, characters));
		}

		assertEncodingWithCharacterPrecision(new WGS84Point(39.0247389581054,
				-76.5110040642321), 12, "dqcw4bnrs6s7");

		String geoHashString = GeoHash.geoHashStringWithCharacterPrecision(
				point.getLatitude(), point.getLongitude(), 12);
		assertEquals(fullStringValue, geoHashString);
	}

	private void assertEncodingWithCharacterPrecision(WGS84Point point,
			int numberOfCharacters, String stringValue) {
		GeoHash hash = GeoHash.withCharacterPrecision(point.getLatitude(),
				point.getLongitude(), numberOfCharacters);
	...
	private void checkMoveAroundStrip(String direction) throws Exception {
		for (int bits = 2; bits < 16; bits++) {
			...
			GeoHash hash = GeoHash.withBitPrecision(randomLatitude,
					randomLongitude, bits);
			Method method = hash.getClass().getDeclaredMethod(
					"get" + direction + "Neighbour");
			GeoHash result = hash;
	...
	public void testKnownNeighbouringHashes() {
		...
		assertEquals("u302", h1.getEasternNeighbour().getEasternNeighbour()
				.toBase32());
		assertEquals("u1p8", h1.getWesternNeighbour().toBase32());

		assertEquals("sp2j", GeoHash.withCharacterPrecision(41.7, 0.08, 4)
				.toBase32());
	}
	...
	public void testKnownAdjacentNeighbours() {
		...
		String[] adjacent = new String[] { "dqcjqf", "dqcjqb", "dqcjr1",
				"dqcjq9", "dqcjqd", "dqcjr4", "dqcjr0", "dqcjq8" };
		assertAdjacentHashesAre(center, adjacent);

		center = "u1x0dfg";
		adjacent = new String[] { "u1x0dg4", "u1x0dg5", "u1x0dgh", "u1x0dfu",
				"u1x0dfs", "u1x0dfe", "u1x0dfd", "u1x0dff" };
		assertAdjacentHashesAre(center, adjacent);

		center = "sp2j";
		adjacent = new String[] { "ezry", "sp2n", "sp2q", "sp2m", "sp2k",
				"sp2h", "ezru", "ezrv" };
		assertAdjacentHashesAre(center, adjacent);
	}
	...
	private void assertAdjacentHashesAre(String centerString,
			String[] adjacentStrings) {
	...
	public void testToLongAndBack() {
		...
		assertEquals(lv + (1 << (64 - hash.significantBits())), hash.next()
				.longValue());
		GeoHash hashFromLong = GeoHash
				.fromLongValue(lv, hash.significantBits());
	...
	@Test
	public void testGetCharacterPrecisionWorksWhenPrecisionIsMultipleOfFive()
			throws Exception {
	...
	public void testStepsBetween() {
		...
		BoundingBoxGeoHashIterator iter = new BoundingBoxGeoHashIterator(
				new TwoGeoHashBoundingBox(bl, ur));
		int count = 0;
		while (iter.hasNext()) {
			iter.next();
			count++;
		...
		assertEquals(steps, lonLess + latLess + latMore + lonMore + inBbox
				- latLessLonMore - latMoreLonLess - 1);
